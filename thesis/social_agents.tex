...
Demonstrator                   & Mode & $\alpha_i$ & Deterministic demonstrator actions \\ \hline
...
\multirow[t]{2}{*}{Pretrained B6} & 1-3  & 0.01, 0.2, 1, 1.5        & No                           \\
                               & 4-6  & 0.01, 0.2, 1             & No                          
\end{tabular}
  \caption[Overview of the experiments for Social Agent I.]{Overview of the experiments for our Social Agent I. We tested different demonstrator, mode, and imitation learning rates $\alpha_i$ combinations. Also, we examined the influence of using deterministic demonstrator actions instead of sampling them.}
  \label{tab:social1-params}
\end{table}
}
\todo[inline]{early stopping, determ. vs not determ: $a_t^d \sim \pi^d(a_t|s_t)$ vs $a_t^d=f^d(\epsilon_t;s_t)$}
As demonstrators, we used either the pre-trained B5 and B6 presented in Section~\ref{sec:pretrained-demos} or the two or four random demonstrators.
...
\center
  ...
  \caption[Experiment results of Social Agent I.]{Test results of Social Agent I. The performance of the baseline SAC agents was only minimally exceeded. Modes 4-6 require a relatively high imitation learning rate to match the baseline's performance.}
...
\subsection{Discussion}
\todo[inline]{Figures alle die gleiche Schriftart, Q-Value vs Q-value}
...
Demonstrator                   & $\alpha_i$                                     & Deterministic policy actions  \\ \hline
\rule{0pt}{3ex}% EXTRA vertical height  
...
\end{tabularx}
  \caption[Overview of the experiments for Social Agent II.]{Overview of the experiments for our Social Agent II. We tested different demonstrator and imitation learning rates $\alpha_i$ combinations. Also, we examined the influence of using deterministic demonstrator actions instead of sampling them.}
...
\center
  ...
  \caption[Policy and Q-network losses and during training of B3 with Social Agent II.]{Policy and Q-network losses during training of B3 for the Social Agent II with imitation learning rates $\alpha_i = 0.15$ and $\alpha_i = 1e^{-4}$. The loss curves with the higher imitation learning rate are overall as expected, while the small rate introduces an increasing policy loss.}
