...
Loss mostly weird for modes 4-6 beside mode 5 \& 6 when using ir 1 (than also similar performance as mode 1-3)

Loss 'weird': at beginn at each batch high, than lowers during this batch, but overall increasing. maybe because probability of taking demonstrator actions for agent to be trained very low, but high alpha (high cerainity to actually not do this), similar pattern in alpha loss (but with negative loss in beginn of batch and than converting to zero during batch) --> overestimated entropy termin --> higher loss
