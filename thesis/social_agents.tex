...
\center
  ...
  \caption[Policy loss and temperature trends during training with SAC-DemoPol.]{Policy loss and temperature trends during B3 training with SAC-DemoPol using mode~2 and mode~5. The loss and temperature decreases as expected when using mode~2, but increases when using mode~5.}
...
The effect disappears if the entropy term~$\mathcal{H}_{M5}$ increases the probability of demonstrator actions sufficiently. Therefore, we assume that the demonstrator actions in the learner's policy are very unlikely, increasing the temperature. As a result, the uncertainty about which action to take enhances, leading to an overall increase in policy loss and, ultimately, poorer performance. Increasing the value term as in $\mathcal{V}_{M1}$ should decrease the overall policy loss. However, based on our results, this does not affect the final performance. 
