...
\noindent
This chapter explains the development, experiments, and results of the social methods we develop. We design four different methods for achieving our goal. The first method employs imitation learning to replicate the behavior of an expert demonstrator. The second and third methods are based on the DB theory. The fourth method uses the MARLISA algorithm to enhance collaborative decision-making. We discuss the implementation of each method and analyze their respective results.

\section{Implementation of Imitation Learning}
Our first method uses imitation learning, for which we use pre-trained demonstrators as experts. We first present our method and results, afterwards we discuss them.
...
As there is a wide range of buildings, we cannot assume that the demonstrators have specified the optimal policy for all the learners. Therefore, we do not imitate the demonstrator transitions directly. Instead, we integrate them into the SAC training of the learners. To achieve this, we fill the PRB with the demonstrator transitions for each agent at the beginning of the training. We use either the transitions from D5 or D6, but not both simultaneously. Figure~\ref{fig:prb-kpis} visualizes the results compared to the baseline SAC agents.
...
The SAC agents using the demonstrator transitions perform worse than the baseline agents on all KPIs. However, using transitions from D6 results in better scores compared to using transitions from D5. This is contrary to the performance of both demonstrator buildings, as described in the previous chapter. 

We conduct additional experiments using the Deep Deterministic Policy Gradient (DDPG) algorithm to confirm that the SAC algorithm is not responsible for the poor results. DDPG is, like SAC, a model-free, off-policy algorithm for solving continuous action space problems, but it learns a deterministic policy \cite{lillicrap2015continuous}. We first train the training buildings without demonstrator transitions using DDPG and the same hyperparameters, reward function and early stopping method as the SAC baseline agents. Then, we train the buildings, adding the same demonstrator transitions as before in their PRB. 

Figure~\ref{app:prb-ddpg-kpis} shows the results. The DDPG agents without imitation learning achieve worse results than the SAC baseline agents in all KPIs, with a decline of about 2.5~\% in fossil energy savings. On the contrary, the agents using imitation learning perform even worse, consuming more fossil energy than without using the battery.

\subsection{Discussion of Imitation Learning}
The policy loss values during training with demonstrator transitions increase for all agents instead of decreasing as expected. Figure~\ref{fig:prb-losses} illustrates these trends for B3 and the demonstrator transitions of D5 using the SAC algorithm. The curves are similar for the other training buildings, D6, and the DDPG algorithm. This development suggests that with higher probability, the agents take actions with a low, possibly even negative, estimated Q-value. As a result, the agents choose 'bad' actions with greater certainty, ultimately leading to poorer performance. 
...
We do not optimize the hyperparameters for the DDPG agents, suggesting that better results could be achieved. However, the increasing policy loss when using the DDPG algorithm and demonstrator transitions supports the thesis that imitation learning is unsuitable for our problem setting. Despite this, the DDPG algorithm does not show better results or more stable training than the SAC algorithm, so we stuck with SAC for further experiments.

Our objective is not to replicate the behavior of a demonstrator in our buildings but rather to learn from it and adopt profitable practices. Given the diversity of our buildings, not all demonstrator actions may be universally effective. Moreover, the transitions do not fulfill the i.i.d. assumption made in behavior cloning since the actions are sampled from the learned policy and thus are not equally distributed. Therefore, we focus on the DB approach in our following experiments. This method leverages the actions of the demonstrator more moderately.
...
In our first approach based on DB, we adjust the loss function of the policy update. As described in Section~\ref{sec:decision-biasing}, DB assumes that demonstrator actions are valuable and, therefore, the probability of choosing those actions or their Q-value should be increased. To accomplish this, we test six different operation modes using an imitation learning rate (ILR) that we also optimize. Also, we investigate the impact of sampling demonstrated action from the demonstrator policy~$a_t^d \sim \pi^d(a_t|s_t)$ versus using the deterministic action (i.e., the learned mean)~$a_t^d=f^d(\epsilon_t;s_t)$.
...
For each gradient step during training, we incorporate an extra policy update step for each demonstrator~$d \in D$, using the social objective function~$J_{\pi}^{social}(\phi)$. This social objective comprises different entropy and value terms based on the mode but is fundamentally similar to the conventional objective. Appendix~\ref{sec:app-algos}, Algorithm~\ref{app:sac-social-1} states the training algorithm of SAC-DemoPol. The only difference to the classic SAC algorithm are lines 14-16.

When operating in mode~1, we use the actions sampled from the demonstrator policy~$a_t^d\sim f^d_{\phi}(\epsilon_t;s_t)$. The estimated Q-value in the value term increases by adding a fraction of the absolute value using the ILR~$\alpha_i$:

\begin{equation}
	\mathcal{V}_{M1} = Q_{\theta}(s_t, a_t^d) + \alpha_i|Q_{\theta}(s_t, a_t^d)|.
\end{equation}

It is important to note that we use the Q-functions of the learner and not of the demonstrator. In addition, the entropy term uses the probability of taking the demonstrator action for the demonstrator~$\pi_{\phi}^d(a_t^d|s_t)$:
...
Mode~3 combines both modes by using the entropy term of mode~2 and the value term of mode~1. Modes 4-6 are similar to modes 1-3, but use the probability of taking the demonstrator action in the learner policy~$\pi_{\phi}(a_t^d|s_t)$:
...
We conduct experiments testing different modes and ILRs with various demonstrators. Table~\ref{app:social1-params} lists the conducted experiments, and Figure~\ref{fig:social1-results} shows the achieved fossil~energy~consumption values. The red dashed line represents the comparison value of the SAC baseline agents, while the gray dashed lines show deviations of 0.5~\% compared to the baseline. We consider any deviations greater than 0.5\% to be significant.

As demonstrators, we use either the pre-trained D5 and D6 presented in Section~\ref{sec:pretrained-demos} or the two or four random demonstrators. In the first step, we sample the demonstrator actions from the demonstrator policy instead of using the Gaussian distribution's learned mean. We test the ILRs of $\alpha_i = 0.01$ and $\alpha_i = 0.2$ and compare their results. Modes 4-6 demonstrate lower performance than the baseline, with slightly better results when using the pre-trained demonstrators than the random demonstrators. Modes 1-3 show similar performance to the baseline for all demonstrators. 
...
Based on these results, we test even higher ILRs of $\alpha_i = 1$ for modes 1-3 and 4-6 for pre-trained demonstrators and $\alpha_i = 1.5$ for modes 1-3 and 5. Mode~3 achieves slightly better results with the four random demonstrators and mode~2 with demonstrator D5, with $\alpha_i = 1$. Modes~5-6 achieve comparable results to the baseline with the higher ILRs, while mode~4 still performs worse. Finally, for the two random demonstrators, with $\alpha_i = 0.2$ and deterministic policy actions, modes 1-3 perform similarly to the baseline and modes 4-6 perform worse.

\subsection{Discussion of SAC-DemoPol}
To better understand the performance differences between modes 1-3 and 4-6, we investigate the policy loss of the standard SAC policy update and the change in the entropy value during training. Figure~\ref{fig:social1-losses} visualizes the trends for B3 with demonstrator D5 and $\alpha_i = 0.2$  using operation modes 2 and 5.
...
In mode~2, the policy loss decreases as expected during training, and the entropy converges quickly to a value between 0.5 and 1. However, during training with mode~5, the entropy value increases. At the same time, the policy loss increases, with sudden gains at the beginning of a new batch and an initial reduction in the loss within the batch. This behavior disappears in modes 5 and 6 when using higher ILRs. As in mode~2, the trends show the expected behavior, resulting in better performance. In mode~4, however, no changes can be observed as the ILR increases. The loss curves of modes~1 and 3 are similar to mode~2.

The effect disappears if the entropy term~$\mathcal{H}_{M5}$ increases the probability of demonstrator actions sufficiently. Therefore, we assume that the demonstrator actions in the learner's policy are very unlikely, increasing the entropy. As a result, the uncertainty about which action to take enhances, leading to an overall increase in policy loss and, ultimately, poorer performance. Increasing the value term as in $\mathcal{V}_{M1}$ should decrease the overall policy loss. However, based on our results, this does not affect the final performance. 

To summarize, our approaches increase the probability or Q-values of the demonstrator actions in the agent's policy to be trained. However, they only result in minimal fossil energy savings compared to our SAC baseline. The DB approach is still promising, so in our subsequent experiments, we directly bias the Q-value of the demonstrator actions in the Q-networks.

\section{Design and Evaluation of SAC-DemoQ}
In this chapter, we introduce our third social approach, also built upon the DB theory. We incorporate an extra social Q-Value update using the demonstrator actions to the SAC algorithm, thus naming this agent SAC-DemoQ. First, we present the method and our first experiments. Then, we present experiments with an extended environment state and new building data, and finally, we discuss the results.

\subsection{Methodological Approach and Findings}
In the next step, we incorporate a social Q-value objective into the algorithm and perform an extra update step of the Q-networks accordingly. During each update step in the SAC algorithm, we carry out an additional social Q-value update for both Q-value networks using the following social objective function:
\begin{equation}
...
Figure~\ref{fig:social2-results} shows the results of the first experiments. The graphs show the mean value from the respective experiment with and without additional policy updates, and the shaded area indicates the standard deviation of both experiments. Based on the results, the additional update does not have an evident positive or negative influence on performance. Again, we consider deviations of at least 0.5~\% compared to the SAC baseline significantly better or worse.

First, the SAC-DemoQ achieves better results than the SAC-DemoPol overall. The fossil energy consumption is comparable to or slightly worse than the SAC baseline. Additionally, moderate ILRs produce better results than high or low rates. Random demonstrators achieve best results with rates between 0.03 and 0.1, while pre-trained demonstrators perform best with rates of 0.15 or 0.2. Using random demonstrators does not enhance performance compared to the SAC baseline. SAC-DemoQ with D6 achieves the best results, with fossil energy consumption savings of about 1.5~\% compared to SAC baseline agents.
...
Figure~\ref{fig:social2-results-shifted} compares the fossil energy consumption of the experiments to the values of the SAC baseline agents. The agents perform worse than the SAC baseline at an ILR of 0.1 for both building sets and demonstrators. For higher ILRs, the results are about the same as those of SAC, but no improvements are achieved. The results are not significantly different depending on the demonstrator used.
...
One notable characteristic of this approach is the significant variation in performance between the agents obtained through the early-stopping method and those fully trained. The top-performing agents, which are trained using the deterministic actions of the D6 demonstrator, show a difference of 9~\% in fossil energy savings. However, for the agents sharing their observations, the difference is 5~\%. For the building data of the shifted B3, the difference is only 2~\%.

To explain these observations, we first look at the policy loss and Q-network loss of B3. Figure~\ref{fig:social2-losses}a shows the loss trends when training with the deterministic demonstrator actions of D6 with $\alpha_i = 0.15$. The figure only visualizes the losses of the first Q-Network, but those of the second behave the same. 
...
\center
  ...
  \caption[Policy and Q-network loss trends during training with SAC-DemoQ.]{Policy and Q-network loss trends during B3 training with SAC-DemoQ using ILRs~$\alpha_i = 0.15$ and~$\alpha_i = 1e^{-4}$. The loss curves when using the higher ILR are overall as expected, while the small IRL introduces an increasing policy loss.}
  \label{fig:social2-losses}
\end{figure}
}

During training, the policy loss decreases significantly, even more than the policy loss of the SAC baseline. However, the Q-value loss increases slowly during training, while the expected behavior would be an initial drop and then quick convergence. The agents using shifted building data show a significantly smaller decrease in the policy loss and a minor increase in Q-networks loss (which is not visualized). These observations indicate that the progression of loss is a cause of the significant difference between the early stopping method and the complete training. We hypothesize that the social Q-value update is helpful at the beginning of the training to find a suitable solution quickly. However, it becomes rather harmful during training because the targets start to differ. Adding the shared observations does not change the loss curves. 

When reducing the ILR and using the sampled demonstrator actions from demonstrator D6, the Q-Network losses show the expected behavior (see Figure~\ref{fig:social2-losses}b). However, the policy loss decreases slightly and then increases again, with a high variance overall. The loss curves show a similar trend to the moderate rate when increasing the ILR to 0.8. However, the policy loss decreases even more. We assume the Q-values are overestimated, so worse actions are rated too high. 

Finally, for the generated shifted building data set, we do not shift the capacity of the batteries. Thus, an identical behavior of the individual buildings would lead to different results. However, the fact that the agents for the shifted B3 buildings reach better results than the shifted B5 buildings goes along with the better performance of the D3 demonstrator. 
...
In the next phase, we aim to enhance the collaboration among agents by implementing the MARLISA algorithm. The algorithm, as explained in Section~\ref{sec:marlisa}, boosts cooperative behavior among the agents by exchanging information. A key difference between the setup of MARLISA in the paper and ours is that in MARLISA, buildings can share any excess energy produced within the neighborhood, i.e., all trained buildings together. Further, the agents control the charging and discharging of the storage tank for domestic hot water and chilled water rather than the battery. 

In our experiments with the MARLISA algorithm, we focus on exploring different reward functions and incorporating building-specific observations into the state space. However, we achieve no results comparable to the baseline's performance. Therefore, we summarize our experiments below without going into the details.

First, we use the SAC baseline reward function and the MARLISA reward function. We hope collaborative work leads to better and more stable results with collective reward functions, as the Tolovski and fossil penalty rewards. These collective reward functions penalize too much energy consumption from the grid of all buildings together to reduce fossil energy consumption, so we conduct further tests using these. 

However, these reward functions show inferior performance compared to the baseline. Therefore, we experiment with the direct minimization of the collective fossil energy consumption~$FE_{con}$ or the total energy purchase from the grid~$E_{pos}$. We combine these rewards with the baseline reward~$r_t^b$ with~$\eta=0.5$. We refer to the first element as the collective reward and the second as the individual reward.

To modify the influence of both components, we subtract or multiply the collective component and scale the collective component by~$\frac{1}{|B|}$, where $B$ are the training buildings. Since the individual reward is always negative, the total reward remains negative.

Furthermore, we use the energy consumption of the considered building~$e_b$ instead of the baseline reward as an individual reward, combined with the collective fossil energy consumption. We also test the exponents used in the MARLISA paper and the scaling factors~$0.01$.
...
\end{equation}
However, with fossil energy consumption savings of 0.985 compared to no battery, the performance is significantly worse than the baseline. Including shared observations leads to even more unstable training and, therefore, does not improve the results.

\subsection{Discussion of MARLISA}
Vazquez et al. calculate the KPIs compared to the performance of the RBC, and not in comparison to without using any storage as we did. The results in the paper show that the MARLISA agents outperform the RBC in all metrics, with the smallest improvement in minimizing net electricity demands at only about 1~\% \cite{vazquez2020marlisa}.

However, the authors do not evaluate the performance of the RBCs individually. Using our data, the RBC agents perform significantly worse than without using a battery (see Figure~\ref{fig:sac-kpis}). If this is also true in the original paper, then the performance of the MARLISA agents might not be as impressive as it seems. Accordingly, the algorithm may not lead to a significant energy saving compared to not using the storage. Therefore, based on our experiments, the classic SAC algorithm is more suitable than the MARLISA algorithm for minimizing fossil energy consumption.