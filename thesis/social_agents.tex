...
\noindent
\todo[inline]{Ziel dieses Kapitels ist eine Einf\"uhrung in die Thematik BlaBlaBla ..., IRGENDWO AM ANFANG einzelne besitmmte gebäude als B\{id\}, e.g. B6 for building 6}
...
DDPG baseline and with demonstrator transitions not tuned, tuning could improve their performance. but increasing loss still indicates that imitation learning not good method for our goal of fossil energy consumption reduction. since DDPG baseline (without tuning) worse results than SAC we stay with SAC for further methods of decision biasing
