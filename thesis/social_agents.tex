...
Tried reward functions presented in \ref{sec:reward} for comparison (Tolovski also has collective goal, same for fossil penalty reward $r_{fossil_t}^b$)

Then tried to directly minimize fossil energy consumption $E_{used_f}$

Then tried to combine fossil energy consumption or district energy consumption with asocial final rewaed (always with scaling $\eta=0.5$): subtract it from $r_t^b$ (complete or scaled by $\frac{1}{|B|}$), scale $r_t^b$ with this (fossil) energy consumption

Also based on MARLISA reward: (tried this too), scale building energy consumption by 0.01 (oder auch nicht) and use the squared one ans scale this by consumed fossil energy

Use negative squared standard reward and scale by fossil consumption



\subsection{Das wird so nicht abgegeben lol}
\begin{equation}
	r = -E_{used_f}
\end{equation}
\begin{equation}
