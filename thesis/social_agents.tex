...
\end{align*}
with the entropy term maximizing randomness~$\mathcal{H}_{SAC}$ and the value term maximizing the estimated Q-Value of the action~$\mathcal{V}_{SAC}$. 
...
We use the Q-functions of the learner and not of the demonstrator. In addition, the entropy term uses the probability of taking the demonstrator action for the demonstrator~$\pi_{\phi}^d(a_t^d|s_t)$:
...
Furthermore, we use the energy consumption of the considered building~$e_b$ instead of the baseline reward as an individual reward, combined with the collective fossil energy consumption. We also test the exponents used in the MARLISA paper and the scaling factor~$0.01$.
