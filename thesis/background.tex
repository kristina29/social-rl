...
\end{figure}
\noindent
Reinforcement Learning (RL) is a machine learning method that, in contrast to supervised and not-supervised methods, does need labeled data for learning. Instead, it explores in an environment the possible actions and aims to maximize the long-term reward. Figure \ref{fig:rl_overview} shows the general RL approach. Formally, at each time step $t$, the environment is in a state $s_t$ that is observed by the agent via an observation $o_t$. This observation may not include the complete state. Based on this observation, the agent takes an action $a_t$, which determines the next state $s_{t+1}$ of the environment. The action is determined by the policy $\pi=p(a_t|o_t)$, which may also be deterministic. Also, as reaction to the received action, the environment sends an reward signal $r_t$ to the agent. The policy of the agent is trained with the goal, to maximize the discounted total reward. 

\todo[inline]{TODO Markov Decision Process}
