...
The temperature parameter~$\alpha$ is essential for the success of SAC and needs to be tuned for each task individually. Even during training of one single task, it is advantageous if~$\alpha$ is adjustable depending on the certainty of the policy about the best action. Thus, Haarnoja et al. present a solution for adjusting the temperature parameter automatically, introducing an additional objective for learning the temperature value $J(\alpha)$.
