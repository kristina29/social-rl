...
\center
  ...
  \caption[Locations in NY State from which weather data was used.]{Locations in NY State from which weather data was used. the location with the green border is the initially only used one.}
...
\right..
\end{equation}
At any given time step $t$, $C^b_t$ represents the capacity of the battery for building $b$, while $SOC^b_t$ represents the state of charge of the same battery. The ratio of $SOC^b_t$ to $C^b_t$ calculates the percentage state of charge. 
%Table~\ref{tab:solar-penalty-reward} indicates that t
...
Furthermore, we tested a linear combination $r_t^b$ between these two reward functions, where the parameter $\eta$ determines the influence of the two functions:
\begin{equation}
r_t^b = \eta r_{price_t}^b + (1-\eta)r_{solar_t}^b.
\end{equation}
...
\begin{equation}
r_{tolovski_t}^b= -(E_{r,grid}-E_{net_{pos}})^2.
\end{equation}
...
We optimized the baseline SAC and social agents by training them for only six buildings - 3, 5, 7, 8, 11, and 17. This approach has two advantages. Firstly, training only six agents reduces the training time. Secondly, after the optimization is complete, we can evaluate our results using the other buildings.
...
\rule{0pt}{3ex}%  EXTRA vertical height  
\multirow[t]{2}{*}{Reward Function} & \multirow[t]{2}{*}{$r_{price_t}^b$} &  %See Section \ref{sec:reward} 
$r_{price_t}^b, r_{solar_t}^b, \mathbf{r_{t}^b}$ \\ 
& & (with $\eta \in \{0.25,  \mathbf{0.5}, 0.75\}$), \\ & & $r_{fossil_t}^b, r_{tolovski_t}^b$\\
\rule{0pt}{3ex}%  EXTRA vertical height  
...
\end{tabularx}
\caption[Optimized hyperparameters of the baseline SAC agent.]{Optimized hyperparameters of the baseline SAC agent with the initial value and all tested values. The value that achieved the best results in terms of fossil consumption is printed in bold. The hyperparameters were optimized step by step, so no grid search optimization was performed.}
\label{tab:optimized-sac}
\end{table}

In Table~\ref{tab:optimized-sac}, you can find the hyperparameters we optimized, along with their initial values, all tested values, and the value we ultimately chose (in bold). We selected the value that resulted in improvements of at least 0.5 \% in fossil energy consumption KPI compared to the previous best agent.

First, we found that increasing the number of training episodes does not increase the performance of the agent, so we kept the initial two episodes. We discovered that the agent's performance improved when we weighted the price by 20 times the fossil energy fraction (see Section \ref{sec:prices}). Normalizing the price between 0 and 1 did not increase the performance but led to more stable results, so we implemented this change. Initially, we used only one location as the source of weather data (highlighted with green border in Figure~\ref{fig:weather-locations}). However, using the median values from eight different locations as described in Section \ref{sec:weather-data} produced better results. Autotuning the temperature parameter \cite{haarnoja2018soft} also reduced fossil energy consumption. We then obtained the best results using the linear combination of the price and solar penalty rewards, with a weighting factor of $\eta=0.5$. We first included all available variables in the state of the environment. An overview of these is given in Appendix~\ref{app:observation-space-sac}. However, removing all 6h and 12h forecasts of the weather and price variables improved the performance of the agent. Increasing batch size, gradient clipping and Kaiming initialization did not significantly affect the fossil energy consumption. The same was true for reducing the discount factor and using the L2 loss instead of the L1 loss.
...
Lastly, we implemented the early stopping method. For this, we evaluated the performance of the agent every 168 time steps during the training until the end of the exploration phase (see CityLearn for more details), i.e., once per week in the data. To achieve this, we used deterministic actions on the complete data set to allow the agent to act and calculate the KPIs. We kept track of the training state at which the agent achieved the best value in fossil energy consumption, along with the final training state.

\subsection{Performance}
The KPI values obtained by the final SAC agent, the best SAC agent using the early stopping method (SAC Best) and the RBC agent are visualized in Figure~\ref{fig:sac-kpis}. The figure shows that the RBC agent performs worse than if no battery was available in all KPIs. This could be because the RBC agent does not utilize solar energy during the day to charge the battery but instead discharges it.
...
Both the SAC Best and the SAC Agent achieve values smaller than 1 in all KPIs, indicating lower values in the corresponding cost functions when using the battery compared to without it. While SAC Best performs slightly better than SAC in all KPIs, the exact differences are not discussed. The most significant improvement is the use of available solar energy produced by the buildings' PVs, with SAC agents utilizing almost 25 \% more of the produced energy when using the battery than without it. Moreover, there is an increase of about 2.5 \% in the use of available renewable energy from the grid and approximately 2 \% overall available renewable energy utilization (for SAC Best). For the most relevant KPI, the SAC Best agent saves about 7 \% of the absolute fossil energy consumed.
